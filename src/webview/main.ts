...
function renderRows(): void {
  ...
  for (let i = first; i < last; i++) {
    ...
    el.className = row.isHead ? 'row head' : 'row';
    ...
    el.addEventListener('click', () => vscode.postMessage({ type: 'selectCommit', sha: row.sha }));
    frag.append(el);
  }
...
window.addEventListener('message', (event: MessageEvent<HostMessage>) => {
  ...
  switch (message.type) {
    ...
    case 'done':
      statusEl.textContent = `${message.total.toLocaleString()} commits in ${message.elapsedMs} ms`;
      schedule();
      break;
